...
get_solana_slot() {
  ...
  while [[ $attempt -le $max_attempts ]]; do
    ...
    if [[ $attempt -lt $max_attempts ]]; then
      log_warning "Attempt $attempt: no slot found, retrying in 1 second..." >&2
      sleep 1
    fi
...
extract_slot() {
  ...
  while [[ $attempt -le $max_attempts ]]; do
    # Get the LAST slot number from kafka send message lines
    slot=$(extract_slot_from_log_file $log_file_path)
...
main() {
  ...
  if [[ -z "$log_file_path" ]]; then
    log_error "No log file path found for topic ($current_topic). Make sure this topic if declared properly"
    return 1
  fi

  log_info "found log_file_path to be $log_file_path"
  ...
  local latest_processed_slot=$(extract_slot $log_file_path)
...
check_unhealthy_topics() {
  ...
  for topic in "${unhealthy_topics[@]}"; do
    ...
    if [[ -z "$log_file_path" ]]; then
      log_error "No log file path found for topic ($topic). Make sure this topic if declared properly"
      return 1
    fi
